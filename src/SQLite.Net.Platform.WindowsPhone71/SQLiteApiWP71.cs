...
namespace SQLite.Net.Platform.WindowsPhone71
{
    public class SQLiteApiWP71 : ISQLiteApi
    {
        ...
        public IDbStatement Prepare2(IDbHandle db, string query)
        {
            ...
            var pzTail = string.Empty;
            int r = Sqlite3.sqlite3_prepare_v2(dbHandle.InternalDbHandle, query, Encoding.UTF8.GetByteCount(query), ref stmt, ref pzTail);
                //(dbHandle.InternalDbHandle, query, out stmt);
